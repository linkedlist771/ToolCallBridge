...
# Define _LONG_INFO for integer range validation
class _LongInfo:
    min = -sys.maxsize - 1
    max = sys.maxsize
...
# Define BeamSearchParams class
class BeamSearchParams:
    def __init__(self, beam_width: int, max_tokens: int, ignore_eos: bool = False,
                 temperature: float = 1.0, length_penalty: float = 1.0,
...
# Define SamplingParams class
class SamplingParams:
    def __init__(self, n: int = 1, best_of: Optional[int] = None, presence_penalty: float = 0.0,
                 frequency_penalty: float = 0.0, repetition_penalty: float = 1.0, temperature: float = 1.0,
                 top_p: float = 1.0, top_k: int = 0, min_p: float = 0.0, seed: Optional[int] = None,
                 use_beam_search: bool = False, length_penalty: float = 1.0, early_stopping: bool = False,
                 stop: Optional[Union[str, list]] = None, stop_token_ids: Optional[list] = None,
                 include_stop_str_in_output: bool = False, ignore_eos: bool = False,
                 max_tokens: int = 16, min_tokens: int = 0, skip_special_tokens: bool = True,
                 spaces_between_special_tokens: bool = True, truncate_prompt_tokens: Optional[int] = None):
        self.n = n
...
class EmbeddingCompletionRequest(OpenAIBaseModel):
    ...
    def to_pooling_params(self):
        return PoolingParams(
            dimensions=self.dimensions, additional_data=self.additional_data
        )
...
class ScoreRequest(OpenAIBaseModel):
    ...
    def to_pooling_params(self, *, use_cross_encoder: bool = False):
        return PoolingParams(
            use_cross_encoder=use_cross_encoder, additional_data=self.additional_data
        )
